...
object XmlDocs {
  ...
  /** Loads XML config by path; resolves via Resources.load.
   *
   * @param path the config path (e.g. classpath resource)
   * @return the parsed Document
   */
  def load(path: String): Option[Document] = {
    parseConfig(Resources.load(path))
  }
   ...
   */
  private def parseConfig(urls: Iterable[URL]): Option[Document] = {
    if (urls.isEmpty) {
      None
    } else {
