...
object MetaModelCodec {
  ...
  private def readDefault(in: DataInputStream, pool: Array[String], paramClazz: Class[_]): Option[Any] = {
    in.readUnsignedByte() match
      ...
      case DEnum =>
        val name = nameOf(in.readUnsignedShort(), pool)
        if classOf[Enum[_]].isAssignableFrom(paramClazz) then
          try
            val m = paramClazz.getMethod("valueOf", classOf[String])
            Some(m.invoke(null, name))
          catch
            case e: Exception =>
              System.err.println(s"WARN beaninfo: cannot resolve enum value $name for " + paramClazz.getName + ": " + e.getMessage)
              None
        else
          System.err.println(s"WARN beaninfo: enum default on non-enum type " + paramClazz.getName)
          None
