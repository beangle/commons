...
/** MetaModelJson 调试导出测试（只出不入，无反向解析）。 */
class MetaModelJsonTest extends AnyFunSpec, Matchers {

  it("renders all sections of a ClassMeta as json") {
    val cm = BeanMetaConverter.from(BeanInfos.of(classOf[CodecSample]))
    val json = MetaModelJson.toJson(cm)
  ...
  it("renders methods with params") {
    ...
    val json = MetaModelJson.toJson(BeanMetaConverter.from(bi))
    json should include("\"methods\":[{\"name\":\"code\"")
    json should include("\"params\":[]")
  }

  it("produces parseable json") {
    val json = MetaModelJson.toJson(BeanMetaConverter.from(BeanInfos.of(classOf[CodecSample])))
    Json.parse(json) shouldBe a[JsonObject]
  }

  it("binary form is much smaller than json form") {
    val cm = BeanMetaConverter.from(BeanInfos.of(classOf[CodecSample]))
    val binary = MetaModelCodec.encode(cm).length
    val json = MetaModelJson.toJson(cm).getBytes(StandardCharsets.UTF_8).length
