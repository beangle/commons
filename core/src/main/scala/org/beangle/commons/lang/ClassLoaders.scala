 ...
 */
object ClassLoaders {
  private val buildins = Map("Boolean" -> classOf[Boolean], "Short" -> classOf[Short], "Int" -> classOf[Int],
    "Long" -> classOf[Long], "Char" -> classOf[Char], "String" -> classOf[String], "Float" -> classOf[Float],
  ...
  def loadClass(className: String, classLoader: ClassLoader = null): Class[_] = {
    ...
    if (buildins.contains(className)) buildins(className) else loader.loadClass(className)
  }
